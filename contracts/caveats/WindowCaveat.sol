...
import {IWindowCaveat} from "../interfaces/IWindowCaveat.sol";
...
abstract contract WindowCaveat is IWindowCaveat {
    ...
    function toSchedule(
        ...
        if ($duration == 0) revert WindowLackingDuration();

        /// @dev Must support at least one day.
        if ($daysOfWeek == 0) revert WindowLackingDays();

        /// @dev Prevent weird overlapping windows.
        if ($duration > $repeatsEvery)
            revert WindowLackingSufficientRepeatsEvery();
    ...
    function toWindows(
        ...
        for (startTime; startTime < $cursor; ) {
            ...
            $windows[(startTime / repeatsEvery) % $n] = _toWindow(
                startTime,
                duration,
                daysOfWeek
            );
    ...
    function toWindow(
        uint256 $schedule
    ) external pure returns (Window memory $window) {
        /// @dev Get the schedule details.
        (
            uint32 startTime,
            uint32 repeatsEvery,
    ...
    function _toWindow(
        ...
        uint32 $duration,
        uint8 $daysOfWeek
    ) internal pure returns (Window memory $window) {
        ...
        /// @dev Load the stack.
        $window.periods = new Period[](daysInWindow);
        ...
        for (daysInWindow; daysInWindow >= 0; daysInWindow--) {
            ...
            $window.periods[daysInWindow].startTime = $startTime > topDayTime
                ? $startTime
                : topDayTime;
    ...
    function _isWithinWindow(
        ...
        uint32 currentWindowOpen = $startTime +
            (timeElapsed / $repeatsEvery) *
            $repeatsEvery;

        /// @dev Ensure the current time is within the current window.
        return
            currentTime >= currentWindowOpen &&
